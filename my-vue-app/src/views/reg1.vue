...
const rules = reactive({
  ...
  confirmPassword: [
    { 
      validator: (_, v, cb) => {
        if (!v) return cb(new Error('不能为空'))
        if (v !== form.password)  return cb(new Error('两次输入不一致'))
        cb()
      },
      trigger: ['blur', 'change']  // 增加change事件触发 
    }
  ],
  real_name: [
    { required: true, message: '必填项', trigger: 'blur' },
...
// 表单提交相关方法
const formRef = ref() // 确保已暴露formRef引用 
...
const submitForm = async () => {
  try {
    ...
    ElMessage.success(' 验证通过，开始注册...')
    
  } catch (error) {
    const firstErrorField = Object.keys(error)[0]  // 获取第一个错误字段名 
    ...
    ElMessage.error(firstErrorMessage  || '请完善必填信息')
  }
}
